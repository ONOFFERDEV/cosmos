// M9 B2: verifies that the pipeline/watcher pass owner_scope through accurately to core calls.
// Uses a fake core that records the received parameters — no real core needed. See CONTRACT.md "# M9 확장".
...
/** Fake core for recording calls — implements only the needed methods; the rest throw if unused. */
...
test("runAsk는 검색 히트가 있으면 graphNeighbors에 ownerScope를 전달하고 이웃을 인용 후보로 합류시킨다", async () => {
  ...
  // sources is a deterministic assembly that holds only "cited chunks", so we use an LLM stub that actually cites the neighbor ([2]).
