...
/**
 * Mock that adds an artificial delay to brief calls (cluster agents) and tracks concurrent execution count.
 * Used to verify that the DEEP_AGENT_CONCURRENCY pool never actually exceeds 2 concurrent runs, and that
 * the brief-to-cluster mapping stays stable even when completion order gets shuffled.
 */
...
// (c) Brief parsing — claims without cites are dropped ---------------------------------
...
// (d) No contradiction → synthesis runs once ---------------------------------------------------
...
// (e) Contradiction found → rebuttal once + synthesis twice ------------------------------------------
...
// (f) Total claims across all briefs is 0 → synthesis skipped, blocked ------------------------------
...
// (g) Global renumbering + trace completeness ---------------------------------------------
...
// (h) Mutex — concurrent requests blocked ------------------------------------------------
...
// (i) Concurrency pool — never exceeds 2 concurrent + preserves brief-cluster mapping order -----------
...
test("(i) 클러스터 에이전트 풀은 동시 2개를 초과하지 않고, 브리프-클러스터 매핑 순서를 보존한다", async () => {
  ...
  try {
    ...
    // Assigns delays in reverse order so the item that starts first (A) finishes last — verifies
    // that result mapping stays index-based even when completion order diverges from items order.
    ...
    // Since each of the 4 clusters yields only 1 chunk, the global numbering is 1(A)/2(B)/3(C)/4(D) —
    // all must be cited for numberSources() to keep all 4 sources (uncited chunks are spec'd to be excluded from sources).
    llm.synthesis1Response = {
      sentences: [
        { text: "종합답변A", cites: [1] },
        { text: "종합답변B", cites: [2] },
