// M9: identity→owner_scope 매핑과 /search·/ingest의 소유권 스코프 처리 테스트.
// CONTRACT.md "# M9 확장" mind 절 참고. server-mode.test.ts의 withServer/withAuthServer/
// makeTempDataDir 패턴을 그대로 재사용한다.
...
// server-mode.test.ts와 동일한 패턴: COSMOS_TOKEN을 임시로 설정하고 종료 시 원복한다.
...
// ① ownerScopeFor 3분기 + 고정 admin 네임스페이스(계정 이름과 무관) + null(무인증)
test("ownerScopeFor: admin→shared+admin(이름 무관 고정), member→shared+<이름>, null→shared", () => {
  ...
  // 서로 다른 이름의 admin 계정이 여러 개여도 동일한 고정 네임스페이스를 공유한다.
...
// ② /search: 무토큰→shared, admin 토큰→shared+admin, 클라이언트의 owner_scope 위장은 서버가 덮어쓴다
...
test("POST /search: 클라이언트가 body에 owner_scope를 위장 지정해도 서버가 계산한 값으로 덮어쓴다", async () => {
  ...
  await withAuthServer(deps, "boot-token", async (port) => {
    ...
    assert.equal(res.status, 200);
    assert.equal(calls.length, 1);
    // 토큰 없음 -> null identity -> shared. 클라이언트가 지정한 "shared+bob"은 무시된다.
    assert.equal(calls[0]?.owner_scope, "shared");
  });
});

// ③ member ingest: owner=본인 통과, owner=타인 403, branch_id도 owner도 없으면 403(갱신된 메시지)
