// runGlobalAsk 단위 테스트. 레지스트리(entities) + 클러스터 다이제스트 + search()가
// 하나의 봉투로 병합되고, digest 인용은 digest://<slug> origin을 갖는지 확인한다.
...
test("entities+digests+search가 병합되고 digest 인용은 digest:// origin을 갖는다", async () => {
  ...
  try {
    ...
    // trace는 다이제스트가 있는 클러스터 전부를 consulted로 표시해야 한다(전수 원칙).
