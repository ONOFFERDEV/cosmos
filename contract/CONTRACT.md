...
- **질의 확장 리콜 폴백(2026-07-24)**: fast에서 원 검색이 차단 예상(top rerank_score<0 → shouldSkipLlmCall)이면, 포기 전에 **LLM 1콜로 질의를 한/영 핵심어+관련 하위 키워드·동의어(5~10개)로 확장** 후 전역(cluster_ids 없이·같은 owner_scope) 재검색 1회. 확장 채택·최종 판정 시 **BM25 완화 적용**: 확장 재검색 top rerank<0라도 결과에 실제 BM25 히트(`bm25_rank != null`)가 있으면 LLM에 위임(리랭커가 짧은/혼합 질의의 관련 문서를 저평가 — "coupang" 단독 +1.56인데 키워드 섞이면 음수여도 문서는 관련). **완화는 확장 경로에만** — 원 검색(1차 pass)은 순수 rerank 게이트 유지(정직성). 확장도 rerank<0 & BM25 히트 없으면 insufficient(안티환각 보존). 안전망=LLM 자체 insufficient(트리거 a)+무인용 차단(b). **정상 질의(원 rerank≥0)는 확장 미실행 — 무비용**. 배경=한글 질의가 영문 문서에 bm25=None+음수 rerank로 리콜 실패(실사용 발견, [[cross-language-bm25-recall-reranker-underscore]]). 한계=LLM 확장 비결정적이라 인용 특정성은 확률적(하위 키워드 많을수록 rerank↑, 앵커 반복 강조는 역효과 실측). global은 insufficient 게이트 구조가 달라(엔티티/다이제스트 전수, rerank 컷 없음) 미적용.
- **슬랙 deep 동시 락 완화(2026-07-24)**: deep는 프로세스 전역 뮤텍스 1건(DEEP_BUSY_MESSAGE throw). 슬랙 브리지는 이 오류를 감지 시 원시 오류 대신 부드러운 안내 후 유한 재시도(최대 3회·20s 간격, 폴 루프 비차단 — watermark 즉시 전진), 소진 시 최종 안내. 큐 미구축(경량).
