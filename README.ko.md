...
- **관계 그래프** — 문서 속 `[[위키링크]]`와 frontmatter가 결정론 관계 그래프로(LLM 추출 없음), 검색 확장과 3D 관계선에 사용
- **3D 코스모스 뷰** + 실시간 진행 표시 챗(fast/deep/global 모드)
- **어디서든 질문** — 웹 챗, MCP 브리지(어느 Claude 세션에서든 `cosmos_ask`/`cosmos_search`), 슬랙 DM 브리지(봇에게 `?`로 시작하는 메시지, `??`는 deep 모드 — 답은 스레드로 출처와 함께)
- **이용 계기판** — 모든 질문이 기록(모드·사용자·클라이언트·소요시간)되고 `GET /stats`(관리자)에서 일별 집계로 확인
